...
export class CdkStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    // product catalogue
    const productCatalogLambda = new lambda.Function(
      this,
    ...
    // API Authoriser lambda
    const apiAuthLambda = new lambda.Function(this, "api-auth-lambda", {
      functionName: `${props.subDomain}-api-auth-lambda`,
      runtime: lambda.Runtime.NODEJS_22_X,
      handler: "utility-functions.authoriserHandler",
      code: lambda.Code.fromAsset("functions"),
      environment: lambdaEnvVars,
    });
    ...
    const getImageUploadUrlLambda = new lambda.Function(
      this,
    ...
    // Create the Lambda authoriser
    const authoriser = new apigw.TokenAuthorizer(this, "apiTokenAuth", {
      handler: apiAuthLambda,
      identitySource: "method.request.header.Authorization",
      resultsCacheTtl: cdk.Duration.minutes(5), // cache auh results
    });

    // Expose endpoint `/api/healthcheck`
    const healthchckApi = api.root.addResource("healthcheck");
    // Allow `/api/healthcheck` to receive GET requests, and tell it which lambda to trigger whn it does
    ...
    // Add the authoriser to each method
    productsApi.addMethod(
      "POST",
